...
@bot.slash_command(name="classify",
                   description="AI evaluation of OT/NT text",
                   guild_ids=[1388620030358589563])
async def classify(ctx, msg: str):
    response = "New Testament" if model.predict(vectorizer.transform(
        [msg]))[0] else "Old Testament"
    await ctx.respond(f'Classification : {response}')


def chunk_text(text: str, max_len: int = 2000):
    """
    Splits a string into chunks of at most max_len characters.
    """
    chunks = []
...
async def commandment(ctx,msg:str):
    ...
    matches = []
    for i,(ref,verse) in enumerate(commandments.items()):
        if msg.lower() in verse.lower():
            matches.append(f'{ref} : {verse}')        
    if not matches:
